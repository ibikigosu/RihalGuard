...
Extracts invoice fields into structured output with confidence, consistency checks, and review flags.
...
Finance operations teams that need clean draft data before review or downstream entry.

## Not for

Approving invoices, paying vendors, posting to ERP, or overriding review decisions.

## Risk level

`RG-2 — Structured Output`

The agent can produce structured output for review. It must not mutate external systems, send messages, create tasks, approve records, or perform write actions.
...
- Wrong total, missing required field, duplicate indicator, unsupported layout, or low confidence.

Expected evidence:

- field values, confidence, validation status, review reason
