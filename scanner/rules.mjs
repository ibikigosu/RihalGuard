...
const RISK_LEVELS = ["RG-0", "RG-1", "RG-2", "RG-3", "RG-4", "RG-5"];
...
function scoreContract(contract) {
  ...
  const riskIndex = RISK_LEVELS.indexOf(contract.risk_level);
  const riskLevelKnown = riskIndex >= 0;
  const riskyAgent = riskIndex >= 3;
  ...
  return buildResult("contract", gates, contract);
}
...
function buildResult(type, gates, contract, promptSignals = null) {
  ...
  const trustTier = type === "contract" && contract?.risk_level ? contract.risk_level : type === "prompt" ? inferPromptTrustTier(gates) : "n/a";
